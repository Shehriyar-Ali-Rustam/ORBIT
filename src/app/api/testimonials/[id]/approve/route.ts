...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    if (!verifyTestimonialToken(id, 'approve', token)) {
  ...
  try {
    ...
    return NextResponse.redirect(`${APP_URL}/testimonials/thanks?status=approved&name=${encodeURIComponent(existing.name)}`)
  } catch (err) {
